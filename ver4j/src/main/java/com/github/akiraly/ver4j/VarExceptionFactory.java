...
/**
 * An {@link ExceptionFactory} implementation for variables.
 */
@Nonnull
public class VarExceptionFactory extends AExceptionFactory {
	@Override
	public RuntimeException notNullException(Object name) {
		return newException("Variable \"%s\" is null.", name);
	}

	@Override
	protected RuntimeException notEmptyException(String type, Object name) {
		return newException("%s variable \"%s\" is empty.", type, name);
	}
